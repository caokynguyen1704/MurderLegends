...
PackageHandlers.registerServerHandler("sendNotice", function(player, packet)
  World.CurWorld.SystemNotice(2,packet.data, 40) 
end)
PackageHandlers.registerServerHandler("changeShow", function(player, packet)
  once=false
end)

World.Timer(20, 
  function()
    if GameStart==false then GameStart=true end
    PackageHandlers.sendServerHandlerToAll("isStart",{isStart=isStart})
    ...
    elseif(isStart and GameStart)then
      ...
      if playMap==nil then
        ...
        for index=1,2 do
          ...
          playerNum=Game.GetAllPlayersCount()
          players = Game.GetAllPlayers()
          while (teamNum<numPlayerInTeam[index]) do
            
            
            local team_get = Game.GetTeam(3,false)
            local teamList=team_get:getEntityList()
            local numteam=0
            for kkk,vvv in pairs(teamList) do
              numteam=numteam+1
            end
            print("team:"..index)
            print("num:"..numteam)
             print("teamNum:"..teamNum)
            local player=random_elem(teamList)
            
            if player==nil then 
              print("NILLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
                teamList=team_get:getEntityList()
                player=random_elem(teamList)
                for ka,va in pairs(teamList) do
                  print("key:")
                  print(ka)
                  print("value:")
                  print(va)
